...
//  @Override
//  public Iterable<DatasetHandle> getDatasets(String user, DatasetRetrievalOptions datasetRetrievalOptions) throws Exception {
//    if(clients.isEmpty()) {
...
//    List<NamespaceKey> keys = ImmutableList.copyOf(c.listFlights(new Criteria(null))).stream().map(fi -> new NamespaceKey(fi.getDescriptor().getPath())).collect(Collectors.toList());
...
//    return OpResult.CheckResult.UNCHANGED;
//  }
//
//
//
